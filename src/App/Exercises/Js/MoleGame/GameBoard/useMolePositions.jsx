...
export function useMolePositions(molesOption) {
  ...
  }, [molePosition1, molePosition2, molePosition3, molesOption]);

  const moveRandomly = useCallback(
    (index) => {
      if (molePosition1 === index) {
        moveRandomly1(index);
      }
      if (molePosition2 === index) {
        moveRandomly2(index);
      }
      if (molePosition3 === index) {
        moveRandomly3(index);
      }
    },
    [
  ...
  return [sharedPosition, moveRandomly];
}
